...
\begin{document}
	...
	estimation \citeme[bias from ignoring cross-spectra].
	Any extension of univariate methods to nonparametrically model $\S(f)$
	requires a parametrisation that ensures Hermitian positive definiteness
	at every frequency while allowing each matrix element to vary smoothly
	with frequency.

	In this paper we extend the Bayesian P-spline PSD framework
	...
	condition.
	Each block's log-diagonal and the real and imaginary parts of the complex
	off-diagonal components are modelled with penalised B-splines under a
	...
	semidefinite $\Y(f_k)$ and the linearity of the trace.
