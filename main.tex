...
\begin{document}
    ...
    the auto-spectra and the real and imaginary parts of the co-spectra of
	...
	($\mathrm{RIAE}=0.114$, coverage $84\%$), with NUTS and SVI
	posteriors overlaid. Both samplers recover the diagonal PSDs and
	off-diagonal cross-spectra with substantially smoother posterior
	medians than the raw periodogram. The NUTS and SVI medians are nearly
	...
	\begin{itemize}
		...
		reference is available, so a heavily averaged Welch estimate
		($\sim\!10^{3}$ averages) is used as the comparison truth.
	\end{itemize}
	For both datasets we use $K=100$ basis functions for each Cholesky
	component, with knots placed by the adaptive quantile rule described in
	Section~\ref{sec:psplines} so that more knots fall near the steep
	low-frequency rise. We choose $N_{b}$ to give blocks of
	...
	$\sim\!4\times$ increase in wall time. Across durations, the credible
	bands shrink monotonically while remaining centred on the same noise
	model. The 1-month and 6-month fits
	...
	$\eta=0.01$ up to $\eta=1$ at every duration. In this experiment the
	point estimate is therefore much less sensitive to $\eta$ than the
	uncertainty calibration. Second, the
