...
\begin{document}
	...
	\begin{abstract}
		...
		series. The inverse spectral matrix is parametrised through a
		frequency-varying Cholesky factor with log-diagonal terms, \RM{(We model the lower-triangular and diagonal components of the frequency-varying Cholesky decomposition of the inverse spectral density matrix , thereby .. at every frequency. Model flexibility and smoothness for each Cholesky component is induced by a penalized B-spline prior)} enforcing
		Hermitian positive definiteness at every frequency, while smoothness is
		induced by penalised B-spline priors on each Cholesky component.
		...
		stochastic variational approximation. The approach scales linearly with
		the number of frequencies and basis functions. On synthetic VAR(2)
		...
		spectral matrix for time-delay interferometry combinations.
	\end{abstract}
	...
	statistical foundation. Penalised B-splines (P-splines) \citep{EilersMarx1996} represent
	smooth functions as linear combinations of B-spline basis functions with a
	roughness penalty on adjacent coefficients. In a Bayesian formulation this
	penalty corresponds to a hierarchical Gaussian prior on the spline
	coefficients, with the smoothing parameter integrated out. \pmr{Technically, the penalty could be incorporated without the hierarchical structure. Should it not be more appropriate to say something like: ``In a Bayesian formulation, the penalty can be incorporated naturally into the prior distribution of the spline coefficients"} Applied to
	...
	Univariate Bayesian P-spline PSD methods have been applied to GW data
	\citep{maturana-russel_bayesian_2021} \pmr{We didn't apply it to GW data, but I'd like to cite it somewhere}, and recent LISA-specific work has introduced log-spline
	...
	These univariate approaches model each channel independently. \pmr{I'd improve the connection of these sentences with something like: ``This is justified in the idealised equal-arm ..."}The
	idealised equal-arm stationary LISA configuration has vanishing
	off-diagonal cross-spectra between the three TDI channels, but realistic
	noise --- unequal arm lengths, per-MOSA imbalance, and instrumental
	breathing \citep{muratore2021instrumental} --- generates non-trivial
	off-diagonal entries in the $3\times 3$ spectral density matrix
	$\S(f)$. Treating these cross-spectra as zero when they are not can
	degrade sensitivity to stochastic GW backgrounds and bias parameter
	estimation \citep{yi2025biases,yi2026biases2,chandramouli2024grtests}\RM{do these references really look at bias induced by zero cross-spectra? Some look at the effect of ignoring higher order modes in the GW signal. Ref 11 links to a totally different paper on reinforcement learning.}.
	Modelling $\S(f)$ nonparametrically therefore requires a
	parametrisation \RM{nonparametrically and parametrisation in one sentence: maybe " Modelling $\S(f)$ nonparametrically therefore requires a flexible model that ....} that guarantees Hermitian positive definiteness at every
	frequency while allowing each element to vary smoothly with frequency. \pmr{What about this here ``Any extension of univariate methods to nonparametrically model $\S(f)$ requires a parametrisation that ensures Hermitian positive definiteness at every frequency while allowing each matrix element to vary smoothly with frequency."?}

	In this paper we extend the Bayesian P-spline PSD framework \pmr{we can cite here the p-spline paper} to the
	multivariate setting. We parametrise $\S(f)^{-1}$ through its Cholesky
	decomposition \citep{RosenOri2007Aeom, Hu2023}, which factorises the
	estimation problem \RM{...which factorises the posterior density int $p$ independent components that can be sampled in parallel.} into $p$ independent regression blocks that can be
	sampled in parallel and guarantees the Hermitian positive definiteness condition.
    \RM{p undefined. Maybe start paragraph: In this paper we extend the Bayesian P-spline PSD framework to the multivariate $p$-dimensional setting.}
	Each block's log-diagonal and complex off-diagonal
	components are modelled with penalised B-splines under a hierarchical
	smoothing prior \RM{and the real and imaginary part of the complex...}. 
	Inference proceeds through a blocked, coarse-grained
	Whittle likelihood with safe-Bayes $\eta$-tempering~\citep{grunwald2019}
	to stabilise posterior calibration, using the No-U-Turn Sampler
	\citep{hoffman2014_NUTS} whose chains are started from draws of a
	stochastic variational approximation~\citep{hoffman2013_SVI}. 
    \RM{This prior is updated by ... Also, this is quite a long sentence. Maybe full stop after "calibration" (is the the right word?) then:" Each chain is ..."}
	We validate
	the method on a bivariate VAR(2) benchmark against \citet{Liu2023}, a
	three-channel VAR(2) simulation, and a simulated LISA data segment.
	...
	Section~\ref{sec:discussion} \pmr{summarises the results and ...} discusses limitations and future
	directions.

	\section{Multivariate P-splines}\label{sec:psplines}

	\subsection{Likelihood}
	A glossary of the notation used in this section is provided in
	Table~\ref{tab:definitions} (Appendix~\ref{app:notation}). Let
	...
	For stationary series with absolutely summable autocovariances
	$\sum_{h=1}^{\infty}|\gamma_{lm}(h)|<\infty$, the Fourier coefficients
	...
	\end{align}
	where $\d(f_{k})^{*}$ denotes the conjugate transpose and $\S(f_{k})$ is
	...
	spectral leakage, in which case the block periodogram is rescaled
	by the inverse effective noise bandwidth $\mathrm{ENBW}=
	n_{b}\sum_{t}w_{t}^{2}/(\sum_{t}w_{t})^{2}$ (with $n_{b}=n/N_{b}$),
	so that the complex-Wishart mean of $\Y(f_{k})$ remains
	$T_{b}\,\S(f_{k})$ and Eqs.~\eqref{eq:blocked-trace}--\eqref{eq:log-like}
	apply unchanged. \pmr{I'd move the next details to their corresponding sections.} The VAR(2) simulation study of
	Section~\ref{subsec:sim_3d_var2} uses a rectangular window
	($\mathrm{ENBW}=1$); the LISA application of
	Section~\ref{subsec:lisa} uses a Tukey window with shape parameter
	$\alpha=0.1$ ($\mathrm{ENBW}\!\approx\!1.04$).}.
...
g		\label{eq:blocked-trace}
	...
	from summing $N_{b}$ \pmr{assumed} independent single-block periodogram
	contributions:
	\[
	...
	$\u^{(k)}_{\nu}=\sqrt{\lambda^{(k)}_{\nu}}\,\v^{(k)}_{\nu}$.
	Substituting into the trace yields the eigenvector form \pmr{Substituting into the trace in \eqref{eq:blocked-trace} gives the eigenvector form of the blocked likelihood}
	\begin{equation}
		\label{eq:log-like}
		\mathcal{L}_{b}(\d|\S)
	...
	\end{equation}
    \pmr{I'd briefly add here the justification for this}
	...
	Explicitly, for $p=3$ channels (as in the LISA case), the three
	...
	original likelihood; $\eta\to 0$ recovers the prior. Intermediate
	...
	\citet{maturana-russel_bayesian_2021}: the absolute-valued empirical periodogram
	for each Cholesky component is normalised to a probability mass function
	over frequency, and knots are positioned at equally-spaced quantiles of
	its cumulative distribution. Knots therefore cluster where the Cholesky component 
	...
	$\mathcal{O}(p^{2}\,N_{c}\,K)$ floating-point operations:
	$N_{c}$ coarse bins, $K$ basis \RM{Is this K=M_j=M_{jl} ?} functions per bin, and $p^{2}$ spline
	...
	$\hat{R}<1.01$ \RM{does this have name?}, that the bulk and tail effective sample sizes both
	...
	metrics. The relative integrated absolute error (RIAE)
	\begin{equation}
	...
	integrals are evaluated numerically using Simpson's rule, measures the
	bias of the posterior-median estimator $\widehat{S}(f)$ against the
	true spectrum $S(f)$. The empirical coverage at level $\alpha$,
	the fraction of (frequency, matrix-element) pairs for which the truth
	lies inside the pointwise $\alpha$ credible band, averaged over
	realisations, measures the calibration of the credible intervals.
	We use $\alpha=0.9$
	throughout.
	...
	\begin{align}
		\mathbf{S}(f) &= \frac{2}{f_{s}}\,H(f)\,\Sigma\,H(f)^{*}, \\
	...
	realisation is fit with $K=10$ basis functions, while sweeping
	...
	All other settings follow Section~\ref{sec:computation}.

	Figure~\ref{fig:vi_vs_nuts_var3} shows the full $3\times 3$ spectral
	matrix recovered for a representative $n=16{,}384$ realisation
	($\mathrm{RIAE}=0.114$, coverage $84\%$), with NUTS and VI
	posteriors overlaid. Both samplers recover the diagonal PSDs and
	off-diagonal cross-spectra, and their medians are nearly
	indistinguishable, supporting the use of the SVI posterior as a fast
	alternative to NUTS for initial exploration.

	Table~\ref{tab:3d_var2_results} reports a representative slice of the
	$(N_{b},N_{h})$ grid. Coverage is near-nominal (90.8--91.8\%) across
	...
	\begin{figure}
		...
		coverage $87\%$). Each panel shows the raw periodogram (grey), the theoretical
	...
	\citep{zenodoLisaDataset}. \RM{For the LISA applications, do we apply some window (Tukey, Kaiser) to each segment?} \avi{We mention it earlier in a footnote, i think we should move that here ie the window function used.} The data were generated with
	...
	\begin{itemize}
		...
		($S_{XX}\!\approx\!S_{YY}\!\approx\!S_{ZZ}$; off-diagonals equal in
		magnitude) and a closed-form reference is available from the
		analytic TDI transfer of the link-level ASDs.
		\item \texttt{noise5a} --- asymmetric realisation: the six per-MOSA
		ASDs are drawn independently from
		...
		would degrade sensitivity to stochastic backgrounds. No closed-form
		reference is available; a heavily averaged Welch estimate
		($\sim\!10^{3}$ averages) is used as the comparison truth.
	\end{itemize}
	For both datasets we use $K=100$ basis functions placed adaptively to
	track the steep low-frequency rise, $N_{b}$ chosen to give blocks of
	order one week in length, and a fixed safe-Bayes tempering
	$\eta=0.5$. The three TDI transfer-function nulls at $0.030$,
	$0.060$, and $0.090$\,Hz are excised with half-width $1$\,mHz prior
	to fitting. All other settings follow Section~\ref{sec:computation}.

	Table~\ref{tab:lisa_results} summarises convergence and CI-width
	diagnostics for the two datasets at 1-month, 6-month, and 1-year
	observation durations. All six runs converge cleanly ($\hat R<1.01$,
	no problematic divergences). For \texttt{noise4a}, where a closed-form
	analytic spectral matrix is available, the Frobenius-norm relative
	integrated absolute error stabilises at the half-percent level
	(RIAE $\approx 5\times 10^{-3}$) across all durations, indicating that
	the posterior-median tracks the true matrix to high fidelity. For
	\texttt{noise5a}, where no analytic reference exists, we instead report
	uncertainty: the median relative CI width on the diagonal PSDs, which
	is the more informative quantity from a calibration standpoint. The
	relative CI width contracts from $\approx\!4.6\%$ at 1\,month to
	$\approx\!1.1\%$ at 1\,year --- a reduction of roughly $4\times$,
	consistent with the $\sqrt{T}$ scaling expected for variance-dominated
	Whittle inference. Coherence-band CI widths halve from
	$\approx\!2.4\%$ to $\approx\!0.6\%$ over the same range. The two
	datasets are virtually indistinguishable in CI width, confirming that
	the per-MOSA noise asymmetry of \texttt{noise5a} does not penalise
	...
	\begin{table}[H]
		...
		\caption{LISA TDI $X_{2}Y_{2}Z_{2}$ results at fixed safe-Bayes
			...
			asymmetric (\texttt{noise5a}) simulations at three observation
			durations. $N_{b}$ is the number of Wishart time blocks (chosen
			to give $\sim 7$-day blocks: $\lfloor T/(7\,\text{d})\rfloor$) and
			$N_{c}$ is set
			to $1024$. RIAE is the Frobenius-norm relative
			integrated absolute error of the posterior-median spectral
			matrix against the analytic reference (\texttt{noise4a} only).
			$\Delta_{\rm PSD}$ and $\Delta_{\rm coh}$ are the median
			pointwise relative widths \RM{how is that defined} of the 90\% credible bands on the
			diagonal PSDs and the off-diagonal coherences, respectively.
			Runtime is wall-clock seconds for one full SVI+NUTS run on a
			single 4-core CPU node.}
		\label{tab:lisa_results}
		\begin{ruledtabular}
			\setlength{\tabcolsep}{4pt}
			\begin{tabular}{llcccccc}
				Noise & Duration & $N_{b}$ &  RIAE & $\Delta_{\rm PSD}$ & $\Delta_{\rm coh}$ & Runtime \\ \hline
				\texttt{4a} & $1$\,mo & $4$  &  $0.0047$ & $4.6\%$ & $2.4\%$ & $274$\,s \\
				\texttt{4a} & $6$\,mo & $25$ &  $0.0053$ & $1.9\%$ & $1.0\%$ & $331$\,s \\
				\texttt{4a} & $1$\,yr & $52$ &  $0.0050$ & $1.2\%$ & $0.7\%$ & $1012$\,s \\
				\texttt{5a} & $1$\,mo & $4$  &  ---     & $4.7\%$ & $2.4\%$ & $236$\,s \\
				\texttt{5a} & $6$\,mo & $25$ &  ---     & $1.9\%$ & $1.0\%$ & $313$\,s \\
				\texttt{5a} & $1$\,yr & $52$ &  ---     & $1.1\%$ & $0.6\%$ & $1149$\,s \\
			\end{tabular}
		\end{ruledtabular}
	\end{table}
	...
	\begin{figure*}[!t]
		\centering
		\includegraphics[width=0.48\linewidth]{figures/lisa_noise4a_eta0p5.png}\hfill
		\includegraphics[width=0.48\linewidth]{figures/lisa_noise5a_eta0p5.png}
		\caption{Posterior summaries of the LISA TDI
			$X_{2}Y_{2}Z_{2}$ spectral matrix for the symmetric
			\texttt{noise4a} (left) and asymmetric \texttt{noise5a} (right)
			datasets at safe-Bayes tempering $\eta=0.5$, shown at three
			observation durations: $1$ month (blue), $6$ months (orange),
			and $12$ months (green). Diagonal panels show the posterior
			$90\%$ credible bands for the auto-spectra
			$S_{XX},S_{YY},S_{ZZ}$; lower-triangle panels show the
			coherence $|C_{ij}(f)|$ for each pair. Welch reference (grey)
			is overplotted. The CI bands shrink monotonically with
			observation length while remaining centred on the same noise
			model.}
		\label{fig:lisa_eta0p5}
	\end{figure*}

	Figure~\ref{fig:lisa_eta0p5} overlays the three durations for both
	datasets. The diagonal auto-spectra are recovered with sub-percent
	bias and CI widths that contract with $\sqrt{T}$, while the
	off-diagonal coherences --- the cross-channel quantities most
	directly tied to stochastic-background and parameter-estimation
	pipelines~\citep{katz2024globalfit} --- are resolved at the few-percent level by 6 months and
	tightened by a further factor of $\sim 1.7$ at 1 year. Wall-clock
	runtime grows much more slowly than the data volume: a $13\times$
	increase in mission duration ($1$\,mo $\to 1$\,yr) costs only a
	$\sim\!4\times$ increase in wall time. The 1-month and 6-month fits
	($\sim\!280$\,s and $\sim\!330$\,s) are dominated by fixed NUTS
	warm-up and SVI overhead, while the 1-year fit ($\sim\!1000$\,s)
	begins to expose the per-gradient $\mathcal{O}(p\,N_{b}\,N_{c}\,K)$
	cost as $N_{b}$ reaches $52$. Even the 1-year fit completes in under
	twenty minutes on a single 4-core CPU node.
	...
	estimator over-concentrates, and a fixed safe-Bayes tempering
	$\eta=0.5$ recovers credible bands that contract with $\sqrt{T}$
	while keeping the sampler away from the tree-depth-saturated regime
	visible at $\eta\to 1$ for the longest durations
	...
	formulation would in principle make $\eta$ unnecessary while
	retaining the linear scaling and Cholesky decoupling of the
	present method. \RM{I am not sure if this is the right conclusion. Even if you have a debiased Whittle likelihood, you might still end up with the very same steep likelihood and problems in the NUTS sampler.}
	...
	small $\eta$ in the appendix sweep. We expect this to be the
	highest-leverage single change for production LISA pipelines.
	...
	assume \emph{local} stationarity \RM{I think this should be called piecewise not local stationarity}: partition the mission timeline
	...
	The current SVI guide is a low-rank ($r=16$) multivariate Gaussian.
	This is sufficient as an initialiser for NUTS but is too restrictive
	to be used as a final posterior approximation: it cannot represent
	the heavy tails or weak multi-modality occasionally seen in the
	off-diagonal Cholesky weights at small $\eta$. Replacing the
	Gaussian guide with a normalising flow (e.g.\ neural spline or
	masked autoregressive flow) would give SVI the flexibility to serve
	as a stand-alone posterior approximation at a fraction of the
	NUTS wall-clock cost --- attractive for the routine,
	parameter-estimation-grade PSD updates expected over the LISA
	mission lifetime.
	...
	The datasets generated are available available at the public Zenodo repository.
	LISA time series data is available at~\citep{zenodoLisaDataset}. 
	...
	The bivariate $L_{2}$ benchmark in Table~\ref{tab:var2_l2_results}
	is the headline result of this appendix. Sensitivity studies across
	the basis size $K$ and the time-series length $n$ are reported in the
	...
	\begin{figure*}[!t]
		\centering
		\includegraphics[width=0.95\linewidth]{figures/lisa_eta_sweep.pdf}
		\caption{Safe-Bayes $\eta$-sensitivity on the symmetric LISA
			\texttt{noise4a} dataset at three observation durations
			(1\,month, 6\,months, 1\,year). Left: median pointwise relative
			width of the 90\% credible band on the diagonal PSDs. Centre:
			coverage of the 90\% credible band against the analytic
			reference (dashed line is the nominal $0.9$). Right: matrix
			RIAE. Connected circles are converged runs; crosses mark runs
			that hit NUTS maximum tree-depth saturation.}
		\label{fig:lisa_eta_sweep}
	\end{figure*}

	Three features are robust across the sweep. First, the matrix RIAE is
	essentially flat in $\eta$, varying by less than $\pm 5\%$ from
	$\eta=0.01$ up to $\eta=1$ at every duration: the posterior median
	is dominated by the data rather than by the tempering, so the
	point estimate is not the relevant calibration quantity. Second, the
	credible-band width contracts monotonically with $\eta$, by roughly
	an order of magnitude from $\eta=0.01$ to $\eta=1$. The contraction
	is steepest at the longest duration, consistent with the per-bin
	Fisher information growing linearly with $N_{b}N_{h}$ as in
	Eq.~\eqref{eq:fisher-scaling}. Third, coverage tracks CI width: at
	$\eta=1$ coverage is well below nominal at all durations
	($0.10$--$0.34$), while at $\eta=0.01$ it approaches the nominal
	$0.9$ at $1$\,month but remains under-calibrated at longer durations
	even at this aggressive tempering. The choice $\eta=0.5$ used in
	Section~\ref{subsec:lisa} is a compromise that yields useful credible
	bands at all three durations without driving the sampler into the
	tree-depth-limited regime visible at $\eta\geq 0.5$ for the 1-year
	configuration. Reaching nominal coverage at full mission duration
	would require either yet smaller $\eta$ or a more flexible model
	(e.g.\ adaptive coarse-graining around the TDI nulls); we leave this
	to future work.
