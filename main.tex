...
\begin{document}
	...
	\begin{align}
		\label{eq:Whittle likelihood}\mathcal{L}(\d|\S) & \propto \prod_{k=1}^{N}\det(\S(f_{k}))^{-1}\times \nonumber \codelink{https://github.com/nz-gravity/LogPSplinePSD/blob/main/src/log_psplines/pipeline/models.py#L81}        \\
	...
	B-splines,
	\begin{align}
		\log \delta_{jk}^{2}    & = \sum_{m=1}^{K_j}B_{m}(f_{k}) w_{j,m}^{(\delta)}\label{eq:delta_spline} \codelink{https://github.com/nz-gravity/LogPSplinePSD/blob/b8e58fd8aaef0c0179499cf7b6a3c2482667626b/src/log_psplines/psplines/psplines.py#L545}    \\
	...
	Fourth, we sample the warm-started posterior with NUTS. The chains
	still run a standard NUTS warm-up (burn-in) phase that adapts the
	mass matrix and step size and allows the sampler to correct for any
	residual bias or under-dispersion inherited from the variational
	initialisation. 
	...
	The scripts used for the LISA data analysis presented in this work are available at
	\url{https://github.com/nz-gravity/lisa_multivar_logpsplines_study}.
