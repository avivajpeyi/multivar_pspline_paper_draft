...
\begin{document}
	...
	statistical foundation, characterised by an expansion of the PSD in terms of an infinite/large number of basis functions combined with noniformative priors on the coefficients. Examples include Bernstein polynomials with weights induced by Dirichlet process priors in \citep{10.1214/18-BA1126},
    Akima splines with Beta distributions on control frequencies in \citep{BaghiQuentin2023Ugbf}, cosines with a discounted regularized horseshoe prior in \citep{PhysRevD.111.062003}, B-splines with knots and weights induced by Dirichlet process priors in \citep{EdwardsMatthewC.2019Bnsd}. With an a priori unknown and potentially infinite number of basis functions, posterior computation requires trans-dimensional Markov chain Monte Carlo (MCMC) algorithms which are computationally expensive. Avoiding trans-dimensional MCMC,
    penalised B-splines (P-splines) \citep{EilersMarx1996} represent
	smooth functions as linear combinations of a finite but large number of B-spline basis functions with a
	roughness penalty on adjacent coefficients that prevents overfitting. In a Bayesian formulation this
	...
	matrix $\S(f)$. Treating these cross-spectra as zero when they are not
    ...
    \textcolor{red}{RM: include paragraph on other current nonparametric approaches to model XYZ psd matrix:}
    %
    Muratore, Gair and Speri~\citep{PhysRevD.109.042001} model the spectra and the real and imaginary parts of the co-spectra of $\S(f)$ individually using natural cubic splines but do not guarantee that the resulting matrix is positive definite. An alternative approach starts with the six single links, assumes that the dominant secondary noises in the single links, the test mass acceleration noise and the optical metrology system noise, are equal, uncorrelated and characterised by the same univariate spectral densities $S_{TM}$ and $S_{OMS}$. The delay operators that define the XYZ TDI variables as linear combinations of time-shifted single link measurments are approximated as complex phasing operators. This then allows to represent the noise spectral density matrix as a Hermitian positive definite matrix where only the univariate test mass and optical metrology system noise components are modelled nonparametrically using cubic splines \citep{BaghiQuentin2023Ugbf} -- with fixed and known TDI transfer functions. 
    

	In this paper, we model with PSD matrix of the XYZ TDI variables directly by extending the Bayesian P-spline PSD framework
	...
	$\sum_{h=1}^{\infty}|\Gamma_{lm}(h)|<\infty$, \pmr{Is the $ml$ correct? We don't use it later} the Fourier coefficients
	...
	\end{align}
	where $\d(f_{k})^{*}$ denotes the conjugate transpose \pmr{of $\d(f_{k})$} and $\S(f_{k})$ is
	Hermitian positive semidefinite \pmr{a Hermitian positive semidefinite matrix}.
	...
	semidefinite $\Y(f_k)$ and the linearity of the trace. \pmr{Do we explain somewhere why we do this?}
	...
	negligible accuracy cost. \pmr{For this purpose, the block frequencies are partitioned into $N_c$ disjoint subsets $J_c$, that is,
    $\{f_1, \ldots, f_{n/(2N_b)}\} = \bigcup_{h=1}^{N_c} J_h,$
    each containing $N_h$ adjacent frequencies with midpoint $\bar{f}_h$. Then, the per-frequency matrix $\mathbf{Y}(f_k)$ is replaced by the within-bin sum $\mathbf{Y}_h^{*} = \sum_{f \in J_h} \mathbf{Y}(f).$} Partition the block frequencies into
	...
	Explicitly \pmr{In particular}, for $p=3$ channels (as in the LISA case), the three
	factors depend on the parameter subvectors $\bdelta_{1}$,
	$(\bdelta_{2},\btheta_{2})$, and $(\bdelta_{3},\btheta_{3})$
	respectively \pmr{, as follows}:
	\begin{widetext}
	\begin{align}
	...
	definite. \pmr{Do we define somewhere the $\alpha$ and $\beta$ values?}
	...
	$\hat{R}<1.01$ (the Rank-normalized Gelman--Rubin potential scale reduction factor~\citeme[Gelman \& Rubin 1992, arviz stats]), that the bulk and tail effective sample sizes both
	...
	metrics. \pmr{The first is the relative ...} The relative integrated absolute error (RIAE)
	\begin{equation}
	...
	integrals are evaluated numerically using Simpson's rule, \pmr{This metrics measures ...} measures the bias of the posterior-median estimator $\widehat{S}(f)$ against \pmr{relative to} the true spectrum $S(f)$. \pmr{The second metric is the empirical ...} The empirical coverage at level $\alpha$, \pmr{defined as}
	the fraction of (frequency, matrix-element) pairs for which the truth \pmr{true values}
	lies inside the pointwise $\alpha$ credible band, averaged over
	realisations, \pmr{This metric measures ...} measures the calibration of the credible intervals. We use $\alpha=0.9$ throughout.
	...
	\end{align}
    \pmr{These parameter specifications guarantee stationarity of the process.}
    
	The one-sided theoretical spectral density matrix is
	\begin{align}
		\mathbf{S}(f) &= \frac{2}{f_{s}}\,H(f)\,\Sigma\,H(f)^{*}, \text{\pmr{with}}\\
	...
	realisation is fit with $K=10$ basis functions \pmr{across the Cholesky parameters}, while sweeping
	...
	($\mathrm{RIAE}=0.114$, coverage $84\%$), with NUTS and VI \pmr{SVI, no?}
	...
	alternative to NUTS for initial exploration. \pmr{We should note that our method yields smoother results.}
	...
	\begin{figure}
		...
		all elements, while both track the true spectrum closely. \pmr{I'd remove the last sentence and discuss about the medians in the text.}
	...
	\end{itemize}
	For both datasets we use $K=100$ basis functions placed adaptively \pmr{This is not clear to me} to
	...
	$\eta=0.5$. 
	Each block is tapered with a Tukey window
	(shape parameter $\alpha=0.1$, $\mathrm{ENBW}\approx1.04$) before
	computing the DFT to suppress spectral leakage.
	The three TDI transfer-function nulls at $0.030$,
	...
	\begin{table}[H]
		...
		\caption{LISA TDI $X_{2}Y_{2}Z_{2}$ results at fixed safe-Bayes
			...
			pointwise relative widths (median over frequencies of
				$[\hat{S}^{95\%}(f)-\hat{S}^{5\%}(f)]/\hat{S}^{50\%}(f)$)
				of the 90\% credible bands on the
	...
	\begin{figure*}[!t]
		...
		\caption{Posterior summaries of the LISA TDI
			...
			model. \pmr{I'd remove the last sentence and insert it somehow in the text}}
	...
	is dominated by the data rather than by the tempering, \pmr{This is not reported, right?} so the
