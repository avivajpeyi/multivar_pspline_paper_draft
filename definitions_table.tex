\begin{table*}[t]
    ...
    \begin{tabular}{p{0.17\textwidth} p{0.77\textwidth}}
        ...
        $N_l$         & Positive-frequency count on the fine (per-block) grid, $N_l = L_b/2$. This is the number of frequencies analysed when \emph{no} coarse-graining is applied. \\
        $N_c$         & Number of coarse-grain bins; the number of frequencies analysed after coarse-graining, $N_c = N_l / N_h$. \\
        $N_h$         & Number of fine-grid frequencies aggregated into each coarse bin, $N_h = |J_h|$; so $N_c \times N_h = N_l$. \\
        ...
        $\lambda_\nu^{(k)}$, $\v_\nu^{(k)}$ & $\nu$th eigenvalue and unit eigenvector of $\Y(f_k)$, so $\Y(f_k)=\sum_\nu\lambda_\nu^{(k)}\v_\nu^{(k)}\v_\nu^{(k)*}$. \\
        ...
        $\mathbf{T}_k$, $\mathbf{D}_k$ & Cholesky factors in $\S(f_k)^{-1}=\mathbf{T}_k^*\mathbf{D}_k^{-1}\mathbf{T}_k$: $\mathbf{T}_k$ is unit lower-triangular; $\mathbf{D}_k$ is diagonal with entries $\delta_{1k}^2,\ldots,\delta_{pk}^2$. \\
        ...
        $\btheta_j$, $\bdelta_j$ & Parameter vectors for component $j$: $\btheta_j$ collects all $\theta_{jl}^{(k)}$ (for $l<j$, across frequencies); $\bdelta_j$ collects all $\delta_{jk}$ (across $k=1,\ldots,N_l$). \\
        ...
        $\mathbf{w}_j^{(\delta)}$, $\mathbf{w}_{jl}^{(\Re)}$, $\mathbf{w}_{jl}^{(\Im)}$ & Spline coefficient vectors for $\log\delta_{jk}^2$, $\Re[\theta_{jl}(f_k)]$, and $\Im[\theta_{jl}(f_k)]$ respectively; e.g.\ $\log\delta_{jk}^2 = \sum_m B_m(f_k)\,w_{j,m}^{(\delta)}$. \\
        $\mathbf{P}_j$  & Penalty matrix for component $j$: $[\mathbf{P}_j]_{lm}=\int_0^1 B_l''(t)\,B_m''(t)\,\mathrm{d}t$. \\
